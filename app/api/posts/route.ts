...
import { findCoverForPost, type PexelsCover } from "@/lib/pexels";
import { rehostCover, usedPhotoIds } from "@/lib/cover-storage";
...
export async function POST(request: Request) {
  ...
  if (rawCover) {
    ...
    const found = await findCoverForPost(title, tags, await usedPhotoIds(supabase));
    if (found) {
